...
static struct adf5355_init_param adf_init_params = {
    .spi_init                       = &spi_params,
    .dev_id                         = ADF5356,
    .freq_req                       = 1700000000ULL, // Desired output frequency in Hz
...
void setup() {
    ...
    /*/
    Serial.println("Testing ADF write...");
    for(int i = 0; i < 14; i ++) {
        delay(2); // 2ms delay
        Serial.println("Writing...");
        ret = no_os_spi_write_and_read(spi_desc, config_100_mhz[i], 4);
    ...
    */
    ...
    ret = no_os_spi_write_and_read(spi_desc, test_data, num_bytes);

    Serial.println("Done Writing"); 


    //uint8_t test_data[] = {0x35, 0x03, 0x00, 0x76}; // R6
    //uint16_t num_bytes = 4;
    //ret = no_os_spi_write_and_read(spi_desc, test_data, num_bytes);
...
// Config using the ADF5355 Driver
/*
    Serial.println("Initializing ADF5356...");
    int32_t ret = adf5355_init(&adf_dev_struct, &adf_init_params);
    Serial.println("Done Initializing.");

    Serial.println("Setting ADF5356 Frequency...");
    ret = adf5355_clk_set_rate(adf_dev_struct, 0, 1700000000);
    if(ret == 0) {
        Serial.println("Done Setting Frequency.");
    }
    else {
...
int32_t teensy_spi_init(struct no_os_spi_desc **desc, const struct no_os_spi_init_param *param) {
  ...
  digitalWrite(local_desc->chip_select, HIGH); // Idles in the high state
  digitalWrite(ADF_DATA, LOW); // Idle low
